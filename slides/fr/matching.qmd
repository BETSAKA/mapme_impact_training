...
date: "Université d'Antananarivo, Octobre 2024"
format: 
  revealjs:
    theme: solarized
    slide-number: true
...
## Hypothèse d'indépendance conditionnelle {.smaller}
...
On va chercher une unité $\Rightarrow$ Pour évaluer l’effet moyen sur les traités (ATE), cela revient à [comparer]{style="color:blue;"} le résultat $Y1$ de chaque unité traitée avec le résultat $Y0$ de l’unité non traitée ayant exactement les mêmes caractéristiques observables.
...
## Variantes du plus proche voisin {.smaller}
...
## Mécanique de l'appariement {.smaller}
...
$\Rightarrow$ Certains plus proches voisins pourront être en fait éloignés. Or la méthode du plus proche voisin traite de la même manière des couples proches et moins proches.
...
$\Rightarrow$ Par exemple, certains bénéficiaires peuvent avoir plusieurs jumeaux très proches : on peut considérer qu’il est dommage d’en choisir un seul (plus ou moins arbitrairement).

## Choix de la taille de la fenêtre {.smaller}
...
## Combien de variables de contrôle ? {.smaller}
...
## Appariement sur le score de propension {.smaller}
...
Cette propriété diminue le nombre de dimensions des comparaisons, puisque l’on ramène le nombre de variables de conditionnement à une seule et unique variable, qui serait un résumé univarié de l’ensemble des covariables.

Cependant, le PSM n’est parfois non connu et il est donc nécessaire de l’estimer.

## Estimation du score de propension {.smaller}
...
## Restriction au support commun {.smaller}
...
## Test visuel du support commun {.smaller}
...
## Imposer un support commun {.smaller}

### Méthode du min/max

- Pour estimer l’ATT, Dehejia et Wahba (1999) éliminent les unités non traitées ayant un score inférieur au minimum observé sur les unités traitées, c.à.d. *"Jamais traités"*

- Pour estimer l’ATE, il faut également éliminer les unités traitées ayant un score supérieur au maximum observé pour les unités non traitées, c.à.d *"always treated"*

## Quelles variables de conditionnement/contrôle ? {.smaller}
