...
```{r Synthèse données satellitaires, fig.fullwidth = TRUE, eval=FALSE}
if (file.exists("data/carte_mailles.rds")) {
  ...
  write_rds(carte_mailles, "data/carte_mailles.rds")
}
...
```{r Jointure aires protégées et données satellitaires, eval=FALSE}
...
AP_info <- AP_Vahatra %>%
  st_drop_geometry() %>%
  select(nom, an_creation, cat_iucn = cat__iucn, gestionnaire = gest_2)

# Assign protected area status to hexagons
grille_mada_AP <- grille_mada %>%
  mutate(
    AP_touche = map_chr(st_intersects(., AP_Vahatra), ~ pluck(AP_info$nom, .x, 1, .default = NA_character_)),
    AP_inclus = map_chr(st_within(., AP_Vahatra), ~ pluck(AP_info$nom, .x, 1, .default = NA_character_)),
